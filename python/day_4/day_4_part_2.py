...
def solution():
    ...
    # Configurations before first iteration
    field_changed = True
    new_padding_field = padding_field.copy()

    while field_changed:

        # Replace the old field of papers with the updated one
        padding_field = new_padding_field
        new_padding_field = padding_field.copy()

        # Assume nothing has changed so far
        field_changed = False
